...
def train_test_split_multimodal_crossval(concept_name_values_selected,df_labels,trial_num_values_remove,
                                         n_splits=5,
                                         shuffle=True,
                                         random_state=42,**kwargs):
    """train_test_split_multimodal_crossval returns df_labels with integer fields of the form
    'cv_fold_{mode}' indicating stratified crossvalidation folds where the trial is in the testing set.
    df_labels is a pandas.DataFrame instance with binary integer fields,
    'y_xmod' and 'y_MvMM' to indicate crossmodal (xmod) and match vs. mismatch (MvMM) trials, respectively.
    df_labels indicates identies by 'faceName', 'pheeName'.
    df_labels is uniequely indexed by trial.
    trial_num_values_remove are removed explicitely.

    Example Usage:
df_labels = train_test_split_multimodal_crossval(concept_name_values_selected,df_labels,trial_num_values_remove,
                                         n_splits=5,shuffle=True,random_state=42)
    """
    #identify unreasonable trials for removal
    boo_reasonable = df_labels['y_xmod']==df_labels['y_xmod'] # tautologically true
    ...
    for train_index, test_index in skf.split(trial_num_values_face_only, y_face_only):
        ...
        df_labels.loc[trial_num_values_face_only_test,'cv_fold_face_only']=cv_iter-1
    ...
    df_labels['cv_fold_voice_only']=-1
    cv_iter=0
    for train_index, test_index in skf.split(trial_num_values_voice_only, y_voice_only):
        ...
        df_labels.loc[trial_num_values_voice_only_test,'cv_fold_voice_only']=cv_iter-1
    ...
    for train_index, test_index in skf.split(trial_num_values_match, y_inm):
        ...
        df_labels.loc[trial_num_values_inm_test,'cv_fold_match']=cv_iter-1
    ...
    for train_index, test_index in skf.split(trial_num_set_values_mismatch, y_mismatch):
        ...
        df_labels.loc[trial_num_values_mismatch_test,'cv_fold_mismatch']=cv_iter-1
